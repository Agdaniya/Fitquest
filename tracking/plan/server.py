...
USER_ID = "user123"  # 🔥 Change this dynamically in frontend when needed
        ...
        while not stop_camera_event.is_set() and camera_active:
            ...
            if results.pose_landmarks:
                ...
                if exercise_state == "active":
                    ...
                    if current_exercise.get("type", "").lower() == "time":
                        # Time-based exercise
...
FIREBASE_RTDB_URL = "https://fitquest-9b891-default-rtdb.firebaseio.com"
# 🚀 Fetch Exercises From Firestore
def fetch_exercises():
    # First, get the user's fitness level
    user_response = requests.get(f"{FIREBASE_RTDB_URL}/users/{USER_ID}.json")
    user_data = user_response.json()
    
    if not user_data:
        print(f"User {USER_ID} not found in Realtime Database")
        return []
        
    # Extract fitness level, default to "beginner" if not found
    fitness_level = user_data.get("fitnessLevel", "Beginner").lower()
    print(f"User fitness level: {fitness_level}")
    
    exercises = []
    # Get exercises from all categories based on fitness level
    for category in ["strength", "cardio", "flexibility"]:
        response = requests.get(f"https://firestore.googleapis.com/v1/projects/{FIREBASE_PROJECT_ID}/databases/(default)/documents/exercises/{fitness_level}/{category}")
        data = response.json()
        ...
        if "documents" in data:
            for doc in data["documents"]:
                fields = doc["fields"]
                exercise = {
                    "name": fields.get("exercise", {}).get("stringValue", "Unknown Exercise"),
                    "sets": int(fields.get("sets", {}).get("integerValue", 1)),
                    "reps": int(fields.get("reps", {}).get("integerValue", 10)),
                    "rest": int(fields.get("rest", {}).get("integerValue", 30)),
                    "type": fields.get("type", {}).get("stringValue", "Reps"),
                    "unit": fields.get("unit", {}).get("stringValue", "reps")
                }
                exercises.append(exercise)
                
    print(f"Found {len(exercises)} exercises for fitness level {fitness_level}")
    return exercises
...
# 🎯 Start Tracking Exercises
@app.route('/track/start-exercise', methods=['POST'])
def start_exercises():
    data = request.json
    user_id = data.get("user_id", USER_ID)
    
    # Fetch exercises from Firestore or use provided ones
    ...
    # Start tracking with pose estimation
    result = start_tracking(exercises)
    return jsonify(result)

# 🛑 Stop Current Exercise
@app.route('/track/stop-exercise', methods=['POST'])
def stop_exercise():
...
# 🏋️‍♂️ Resume Workout
@app.route('/track/resume', methods=['POST'])
def resume_workout():
    exercises = fetch_exercises()
    if not exercises:
        return jsonify({"error": "No exercises found"}), 404
